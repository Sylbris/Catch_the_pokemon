...
public class SimpleGameClient {
	...
	private static int nextNode(directed_weighted_graph g, int src) {
		int ans = -1;
		Collection<edge_data> ee = g.getE(src); //create a list of edges from src node.
		Iterator<edge_data> itr = ee.iterator(); //create an iterator for this list.
		int s = ee.size();
		int r = (int)(Math.random()*s); //r=0-edge.size;
		int i=0;
		while(i<r) {
			itr.next();
			i++;
